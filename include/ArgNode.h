...
struct ArgNode {
    ...
    ArgNode(const char* arg, int n); //Constructs new node, used to construct head node
    ArgNode* append(ArgNode** tail, const char* arg, int n); //Appends node to end of list, modifies tail pointer to point to new end of list
    ...
    const char* arg; //Key of arg at pos
